...
; Aktueller Menüeintrag für Pause/Fortsetzen
...
; StayOnTop-Hotkey (Strg+Win+Space) - Hidden Feature
Hotkey("^#Space", ToggleStayOnTop)
...
; ============================================================
UpdatePauseMenu() {
    global GlobalPaused, CurrentPauseMenuItem

    if (GlobalPaused) {
        newItem := L("General", "111")  ; Fortsetzen
        iconNum := 7
    } else {
        newItem := L("General", "110")  ; Pause
        iconNum := 4
    }
...
EditRule(rule) {
    ...
    MyEdH := MyGui.Add("Edit", "x263 yp-3 w50 Number", rule.h)
    ...
    MyMatchDDL := MyGui.Add("DropDownList", "x109 yp-3 w120 Disabled Choose" chooseIndex, [L("CaptureWindow", "550"), L("CaptureWindow", "555")])
...
CaptureSpecificWindow(hwnd) {
    ...
    lblH := MyGui.Add("Text", "x245 yp+3 +0x100", "H:")
    MyEdH := MyGui.Add("Edit", "x263 yp-3 w50 Number", curH)
    ...
    MyMatchDDL := MyGui.Add("DropDownList", "x109 yp-3 w120 Choose1", [L("CaptureWindow", "550"), L("CaptureWindow", "555")])
...
; ============================================================
; StayOnTop - Fenster immer im Vordergrund togglen
; ============================================================
...
ToggleStayOnTop(*) {
    ...
    try {
        ...
        if (exStyle & 0x8) {  ; WS_EX_TOPMOST
            ...
            ShowStayOnTopSplash(L("Messages", "949"))
        } else {
            WinSetAlwaysOnTop(true, hwnd)
            ShowStayOnTopSplash(L("Messages", "948"))
        }
    }
}

; ============================================================
ShowStayOnTopSplash(text) {
    global StayOnTopSplash

    ; Vorherigen Splash schließen falls vorhanden
    if (IsObject(StayOnTopSplash)) {
        try {
            StayOnTopSplash.Destroy()
        }
    }

    ; Neuen Splash erstellen
    StayOnTopSplash := Gui("+AlwaysOnTop -Caption +ToolWindow +Border")
    StayOnTopSplash.BackColor := "FFFFCC"
    StayOnTopSplash.SetFont("s10 bold", "Segoe UI")
    StayOnTopSplash.Add("Text", "x15 y8 Center", text)

    ; Größe berechnen und zentriert anzeigen
    StayOnTopSplash.Show("AutoSize Hide")
    StayOnTopSplash.GetPos(, , &w, &h)
    x := (A_ScreenWidth - w) // 2
    y := (A_ScreenHeight - h) // 2
    StayOnTopSplash.Show("x" x " y" y " NoActivate")

    ; Nach 1 Sekunde schließen
    SetTimer(CloseStayOnTopSplash, -1000)
}

; ============================================================
CloseStayOnTopSplash() {
    global StayOnTopSplash
